...
WHY IT MATTERS AT BATCH SCALE: in single-task mode a human watches one handoff and can decline the postflight by hand. In multi-task mode the same unguarded mapping is applied across every task in the dispatch batch in one pass, so a premature completed marking is both more likely and less visible. A multi-phase task with a large file_scope is exactly the shape that produces an early "implemented".

DESIRED BEHAVIOR: a single shared completion-claim gate, applied on the implemented branch in ALL THREE paths (single-task Stage 5, multi-task Stage MT-4, and the hard variant). When dispatch_status="implemented" AND either (a) phases_total > 0 and phases_completed < phases_total, or (b) plan_markers_verified is absent or false: do NOT run the postflight-to-completed update. Instead log a loud, greppable warning naming the task and the evidence, and treat the cycle as continuation-eligible so the next cycle re-dispatches implement rather than declaring the task done.

DESIGN CONSTRAINTS:
- Fail CLOSED on missing evidence, but distinguish the two failure modes. A handoff with phases_total=0 (no phase accounting emitted at all) is NOT the same as phases_completed < phases_total, and the warning text must say which one fired.
- Do NOT weaken the existing drift-detection gate or the partial/blocker routing; this gate is strictly additional.

...
### Phase 1: Retire section injection from the sync re-inject path [NOT STARTED]
...
**Tasks**:
- [ ] In `reinject_loaded_extensions()`, delete the "Re-inject config markdown section" block that
      ...
      Leave the `settings` and `index` re-injection blocks in the same loop untouched.
- [ ] After the extension loop closes — as a sibling of the existing
      ...
      `.claude` and `.opencode` deploy targets are covered by the same call.
- [ ] Check whether `merge_key` and any now-unused local remain referenced after the deletion;
      remove any local that has become dead. Do NOT remove `read_file_string` if other call sites
      in the file still use it.
- [ ] Correct the module header comment block ("Section Preservation: ... merge targets for all
      ...
      preservation applies only to the `.opencode` `OPENCODE.md` path.
- [ ] Correct the `reinject_loaded_extensions()` docstring, which currently names `inject_section`
      among the idempotent merge operations it re-runs.
- [ ] Reference durable anchors only (function names, file paths). Do not cite task numbers in any
      file outside `specs/**`.

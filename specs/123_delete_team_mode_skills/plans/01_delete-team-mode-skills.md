...
### Phase 3: Drop the skills from `manifest.json` and the README tree [NOT STARTED]

**Goal**: Remove the three skills from the extension's declared inventory and from the directory
map that documents it.

**Tasks**:
- [ ] `manifest.json`: remove the three entries from the `provides.skills` array. No
      ...
      with a structured search over the file for the substring `skill-team`.
- [ ] `README.md` (core extension): remove the three lines from the skills directory tree, and
      repair the tree-drawing characters on the entry that becomes the new last child (`└──`).
- [ ] Do not hand-edit `.claude-extensions.json`; it is regenerated by `deploy-headless.sh`.

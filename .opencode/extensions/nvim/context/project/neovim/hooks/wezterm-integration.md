...
**Path**: `.opencode/hooks/wezterm-preflight-status.sh`
...
**Path**: `.opencode/hooks/wezterm-utils.sh`
...
**Path**: `.opencode/tmp/workflow-active`
...
```json
{
  "hooks": {
    "Stop": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "bash .opencode/hooks/claude-stop-notify.sh 2>/dev/null || echo '{}'"
      }]
    }],
    "UserPromptSubmit": [{
      "matcher": "*",
      "hooks": [
        ...
        {
          "type": "command",
          "command": "bash .opencode/hooks/wezterm-preflight-status.sh 2>/dev/null || echo '{}'"
        }
      ]
    }]
  }
...
- **TTS integration**: `.opencode/context/project/neovim/guides/tts-stt-integration.md`

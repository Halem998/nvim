...
dispatch_agent() {
  ...
  #   $2 = prompt                 - Full prompt string to pass to Task tool
...
invoke_agent_fork() {
  ...
  # Calls Task tool WITHOUT subagent_type field
  # Implementation: OpenCode Task tool call
  # This is a conceptual representation; actual invocation is via Task tool in SKILL.md
...
invoke_named_agent() {
  ...
  local prompt="$2"
  local context_json="$3"
  # Calls Task tool WITH subagent_type="$agent_type"
...
invoke_agent_fork() {
  ...
  if [ "$FORK_SUBAGENT" = "1" ]; then
    # Use fork (cache-warm)
    # ... Task tool call without subagent_type ...
  else

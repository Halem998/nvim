...
- **Component Type**: Identify component (command, skill, agent, rule, context, documentation)
  - **Affected Area**: Parse for directory mentions (.opencode/commands/, .opencode/skills/, .opencode/agent/, etc.)
...
# Agents
ls .opencode/agent/subagents/*.md 2>/dev/null | while read f; do
  name=$(basename "$f" .md)
  echo "- $name"
done

...
#
# Usage: bash .opencode/scripts/command-gate-out.sh "$task_number" "$operation" "$session_id"
#
...
if [ -n "$expected_status" ] && { [ "$skill_status" = "implemented" ] || \
  ...
  if [ "$current_status" != "$expected_status" ] && [ "$skill_status" != "partial" ] && [ "$skill_status" != "failed" ]; then
    ...
    bash .opencode/scripts/update-task-status.sh postflight "$task_number" "$operation" "$session_id" 2>/dev/null || \
...
if [ -d "$task_dir" ]; then
  bash .opencode/scripts/validate-artifact.sh "$task_dir" --fix 2>/dev/null || true
fi

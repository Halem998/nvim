...
#   Exported variables do NOT persist across separate Bash invocations in OpenCode.
...
#
# CONTEXT BUDGET DEFAULTS (overridable by the context-tier enforcement mechanism):
...
skill_get_extension_dir() {
  ...
  echo ".opencode/extensions/${ext_name}"
}
...
# See: .opencode/docs/architecture/handoff-schema.md
...
skill_preflight_update() {
  ...
  local operation="$2"
  local session_id="$3"
  bash .opencode/scripts/update-task-status.sh preflight "$task_number" "$operation" "$session_id"
...
skill_validate_artifact() {
  ...
  if [ "$status" != "failed" ] && [ -n "$artifact_path" ] && [ -f "$artifact_path" ]; then
    ...
    if ! bash .opencode/scripts/validate-artifact.sh "$artifact_path" "$artifact_kind" --fix 2>/dev/null; then
...
skill_postflight_update() {
  ...
  case "$status" in
    researched|planned|revised|implemented)
      bash .opencode/scripts/update-task-status.sh postflight "$task_number" "$operation" "$session_id"
      ;;
    *)
...
skill_link_artifacts() {
  ...
  if [ -n "$artifact_path" ]; then
    ...
    jq --arg path "$artifact_path" \
      ...
      specs/state.json > specs/tmp/state.json && mv specs/tmp/state.json specs/state.json
    # Link in TODO.md
    bash .opencode/scripts/link-artifact-todo.sh "$task_number" "$field_name" "$next_field" "$artifact_path" 2>/dev/null || \
...
#
# Schema reference: .opencode/docs/architecture/handoff-schema.md

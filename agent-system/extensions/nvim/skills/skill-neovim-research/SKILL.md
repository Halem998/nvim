...
Validate task_number exists and task_type is "neovim".
...
skill_name="skill-neovim-research"
operation="research"
```
...
Domain-specific context for the neovim-research-agent:
- Neovim plugin documentation (lazy.nvim, nvim-lspconfig, etc.)
- Lua configuration patterns from `.claude/extensions/nvim/context/`
- Local config files under `lua/neotex/`
...
Use Agent tool with subagent_type: "neovim-research-agent".
...
3. **Use WebSearch/Bash (nvim --headless) tools** - Research tools are for agent use only

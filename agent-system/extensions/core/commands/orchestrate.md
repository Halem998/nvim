...
`FORCE_PHASES_FLAG` (default `""`) is read here from the sourced parser and passed into the
Skill delegation context below as `force_phases` (the composable `--research`/`--plan`/
...
**Consolidated Output**: READ
`.claude/context/patterns/orchestrate-batch-results-template.md` now and emit the batch results
using that template. The template MUST be followed exactly — its per-section rendering
conditions are part of the contract, not commentary.

...
- Invoke each task's implementation skill with `session_id={batch_session_id}` — the bare batch
  ...
  This is what makes the mechanized per-phase heartbeat resolve correctly — the task-lock and
  session-registry refresh lives INSIDE `update-phase-status.sh` itself (see
  `context/patterns/task-lock.md`), which derives `session_id` from the per-task lock's own
  `holder.json`. Because that holder was written by this loop's `acquire-retry` using the bare
  `$batch_session_id`, and the batch registry entry was written under the SAME bare id by Step 2,
...
not a gap to "fix" later, and the omission is sound **because** the refresh is now mechanically
guaranteed one layer down: the per-*phase* heartbeat lives inside `update-phase-status.sh` itself
(fired at every phase transition of each dispatched `general-implementation-agent.md` run,
independent of any prose call site — see `context/patterns/task-lock.md`), and is what the
`session_id={batch_session_id}` dispatch-args bullet above makes resolve correctly. The two are
not in tension: this command-level loop needs no heartbeat of its own because every dispatched
task already refreshes both records at a finer grain than any batch-level boundary could.
